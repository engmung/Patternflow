...
}`};var I=2;function R(o){return(o[1]-o[0])/I}var d=[[0,1],[.1,10],[0,4.9],[0,1]];var v=[!0,!1,!1,!0];function x(o=d){return d.map((r,n)=>R(o[n]??r))}var _=/^[ \t]*\/\/[ \t]*@knobs[ \t]+(.+)$/m,w=["Knob 1","Knob 2","Knob 3","Knob 4"];function M(o){let r=o.match(_);if(!r)return null;let n=[null,null,null,null];return r[1].split(",").slice(0,4).forEach((e,t)=>{let a=e.trim();if(!a||a==="-")return;let s=a.match(/^(.+?)\s*=\s*(-?\d*\.?\d+)\s*\.\.\s*(-?\d*\.?\d+)$/);if(!s)return;let i=Number(s[2]),l=Number(s[3]);!Number.isFinite(i)||!Number.isFinite(l)||l<=i||(n[t]={name:s[1].trim().slice(0,14),min:i,max:l})}),n.some(Boolean)?n:null}var k=(o,r)=>Number(((o+r)/2).toFixed(3));function S(o){let r=[...w],n=d.map(([a,s])=>[a,s]),e=n.map(([a,s])=>k(a,s)),t=M(o);return t&&t.forEach((a,s)=>{a&&(r[s]=a.name,n[s]=[a.min,a.max],e[s]=k(a.min,a.max))}),{labels:r,ranges:n,values:e}}var K={width:128,height:64};var B=8,q=512,z=/^[ \t]*\/\/[ \t]*@matrix[ \t]+(\d{1,4})[ \t]*[x×*][ \t]*(\d{1,4})[ \t]*$/m;function A(o){if(!Number.isFinite(o))return null;let r=Math.round(o);return r<B||r>q?null:r}var an=8192*4;function X(o){let r=o.match(z);if(!r)return null;let n=A(Number(r[1])),e=A(Number(r[2]));return n===null||e===null?null:{width:n,height:e}}function P(o){return X(o)??K}function T(o){return o.width>o.height?"landscape":o.height>o.width?"portrait":"square"}var E=typeof p>"u"?{}:p;var u=class{constructor(r,n){this.code="";this.values=[.5,.5,.5,.5];this.ranges=[[0,1],[0,1],[0,1],[0,1]];this.wrap=[!1,!1,!1,!1];this.unitsPerTurn=[1,1,1,1];this.running=!1;this.timers=[];this.onStatus=n,this.frame=document.createElement("iframe"),this.frame.src=r,this.frame.setAttribute("sandbox","allow-scripts"),this.frame.setAttribute("title","Pattern preview"),this.frame.setAttribute("scrolling","no"),this.frame.style.cssText="border:0;width:100%;height:100%;display:block;pointer-events:none;background:#000",this.frame.addEventListener("load",()=>this.sendLoad())}get element(){return this.frame}connect(){this.listener=r=>{if(r.source!==this.frame.contentWindow)return;let n=r.data;n?.type==="pf-ready"?this.sendLoad():n?.type==="pf-status"&&this.onStatus?.({ok:!!n.ok,error:n.error})},window.addEventListener("message",this.listener),this.timers=[0,50,200].map(r=>window.setTimeout(()=>this.sendLoad(),r))}disconnect(){this.listener&&window.removeEventListener("message",this.listener),this.listener=void 0,this.timers.forEach(r=>window.clearTimeout(r)),this.timers=[]}load(r,n,e,t,a){if(r===this.code){this.setKnobs(n,e);return}this.code=r,this.values=n,this.ranges=e,this.wrap=t,this.unitsPerTurn=a,this.sendLoad()}setKnobs(r,n){this.values=r,this.ranges=n,this.post({type:"pf-knobs",values:r,ranges:n})}setRunning(r){r!==this.running&&(this.running=r,this.post({type:"pf-run",running:r}))}sendLoad(){this.code&&this.post({type:"pf-load",code:this.code,knobValues:this.values,knobRanges:this.ranges,knobWrap:this.wrap,knobUnitsPerTurn:this.unitsPerTurn,running:this.running})}post(r){this.frame.contentWindow?.postMessage(r,"*")}};var C=`
:host {
  display: block;
}
...
`;var h=4,L=150,N=160,W="/patternflow_static/pattern-sandbox.html";function m(o,r){if(r.switch_entity||r.select_entity||r.knob_entities)return{switchId:r.switch_entity,selectId:r.select_entity,knobIds:r.knob_entities??[]};let n=o.entities??{},e=Object.values(n).filter(i=>i.platform==="patternflow"),t=r.device_id??e.find(i=>i.device_id)?.device_id,a=e.filter(i=>!t||i.device_id===t),s=a.filter(i=>i.entity_id.startsWith("number.")).sort((i,l)=>D(o,i.entity_id)-D(o,l.entity_id)).map(i=>i.entity_id);return{switchId:a.find(i=>i.entity_id.startsWith("switch."))?.entity_id,selectId:a.find(i=>i.entity_id.startsWith("select."))?.entity_id,knobIds:s}}function D(o,r){let n=o.states[r]?.attributes?.knob;return typeof n=="number"?n:Number.MAX_SAFE_INTEGER}function H(o){if(!o)return null;let r=Number(o.state);return Number.isFinite(r)?r:null}var b=class extends HTMLElement{constructor(){super();this.config={type:"custom:patternflow-card"};this.zones=[];this.knobButtons=[];this.mounted=!1;this.code="";this.slug=null;this.labels=["K1","K2","K3","K4"];this.ranges=[[0,1],[0,1],[0,1],[0,1]];this.local=[50,50,50,50];this.dragging=null;this.dragStartValue=0;this.dragStartY=0;this.active=0;this.lastSentAt=[0,0,0,0];this.pending=[null,null,null,null];this.timers=[null,null,null,null];this.root=this.attachShadow({mode:"open"})}setConfig(n){this.config={preview:!0,show_patterns:!0,...n},this.mounted=!1,this.root.innerHTML=""}set hass(n){this.hassRef=n,this.mounted||this.build(),this.update()}getCardSize(){return this.config.show_patterns?12:8}static getStubConfig(n){return{type:"custom:patternflow-card",device_id:Object.values(n.entities??{}).find(t=>t.platform==="patternflow"&&t.device_id)?.device_id}}static getConfigElement(){return document.createElement("patternflow-card-editor")}disconnectedCallback(){this.sandbox?.disconnect(),this.timers.forEach(n=>n!==null&&window.clearTimeout(n))}build(){let n=document.createElement("style");n.textContent=C;let e=document.createElement("ha-card");e.innerHTML=`
      <div class="stage" part="stage">
        <div class="still">loading\u2026</div>
        <div class="zones">
          ${Array.from({length:h},()=>'<div class="zone"></div>').join("")}
        </div>
    ...
    `,this.root.append(n,e),this.stage=e.querySelector(".stage"),this.zones=Array.from(e.querySelectorAll(".zone")),this.readout=e.querySelector(".readout"),this.knobButtons=Array.from(e.querySelectorAll(".knob")),this.attachGestures(),this.attachStripGestures(),this.attachPower(e),this.mounted=!0}attachPower(n){n.querySelector(".power").addEventListener("change",()=>{let{switchId:t}=m(this.hassRef,this.config);t&&this.hassRef?.callService("switch","toggle",{entity_id:t})})}attachGestures(){let n=this.stage;n.addEventListener("pointerdown",t=>{this.knobsUsable()&&t.pointerType==="mouse"&&(n.setPointerCapture(t.pointerId),n.classList.add("touched"),this.active=this.zoneAt(t),this.dragging=this.active,this.dragStartValue=this.local[this.active],this.dragStartY=t.clientY,this.dodge(t),this.paintOverlay())}),n.addEventListener("pointermove",t=>{if(this.dragging===null){t.pointerType==="mouse"&&(n.classList.add("touched"),this.active=this.zoneAt(t),this.dodge(t),this.paintOverlay());return}let a=(this.dragStartY-t.clientY)/n.clientHeight;this.setLocal(this.dragging,this.dragStartValue+a*100),this.dodge(t)});let e=t=>{if(this.dragging===null)return;let a=this.dragging;this.dragging=null,n.hasPointerCapture(t.pointerId)&&n.releasePointerCapture(t.pointerId),this.flush(a,!0),t.pointerType!=="mouse"&&n.classList.remove("touched")};n.addEventListener("pointerup",e),n.addEventListener("pointercancel",e),n.addEventListener("pointerleave",()=>{this.dragging===null&&n.classList.remove("touched")}),n.addEventListener("wheel",t=>{this.knobsUsable()&&(t.preventDefault(),this.active=this.zoneAt(t),this.setLocal(this.active,this.local[this.active]+(t.deltaY<0?4:-4)))},{passive:!1}),n.addEventListener("dblclick",t=>{if(!this.knobsUsable())return;let a=this.zoneAt(t);this.setLocal(a,50),this.flush(a,!0)})}attachStripGestures(){this.knobButtons.forEach((n,e)=>{n.addEventListener("pointerdown",a=>{this.knobsUsable()&&(a.preventDefault(),n.setPointerCapture(a.pointerId),this.active=e,this.dragging=e,this.dragStartValue=this.local[e],this.dragStartY=a.clientY,this.paintOverlay())}),n.addEventListener("pointermove",a=>{if(this.dragging!==e)return;let s=(this.dragStartY-a.clientY)/N;this.setLocal(e,this.dragStartValue+s*100)});let t=a=>{this.dragging===e&&(this.dragging=null,n.hasPointerCapture(a.pointerId)&&n.releasePointerCapture(a.pointerId),this.flush(e,!0))};n.addEventListener("pointerup",t),n.addEventListener("pointercancel",t),n.addEventListener("dblclick",()=>{this.knobsUsable()&&(this.setLocal(e,50),this.flush(e,!0))})})}zoneAt(n){let e=this.stage.getBoundingClientRect(),t=(n.clientX-e.left)/e.width;return Math.max(0,Math.min(h-1,Math.floor(t*h)))}dodge(n){let e=this.stage.getBoundingClientRect(),t=(n.clientY-e.top)/e.height;t>.55?this.readout.classList.replace("bottom","top"):t<.45&&this.readout.classList.replace("top","bottom")}knobsUsable(){let{knobIds:n}=m(this.hassRef,this.config);return n.length===h}setLocal(n,e){this.local[n]=Math.max(0,Math.min(100,e)),this.paintOverlay(),this.pushKnobsToSandbox(),this.scheduleWrite(n)}scheduleWrite(n){this.pending[n]=this.local[n];let e=performance.now()-this.lastSentAt[n];if(e>=L){this.flush(n);return}this.timers[n]===null&&(this.timers[n]=window.setTimeout(()=>this.flush(n),L-e))}flush(n,e=!1){let t=this.timers[n];t!==null&&(window.clearTimeout(t),this.timers[n]=null);let a=e?this.local[n]:this.pending[n];if(a===null)return;this.pending[n]=null,this.lastSentAt[n]=performance.now();let{knobIds:s}=m(this.hassRef,this.config),i=s[n];i&&this.hassRef?.callService("number","set_value",{entity_id:i,value:Math.round(a)}).catch(l=>this.showNotice(String(l)))}update(){let n=this.hassRef;if(!n||!this.mounted)return;let{switchId:e,selectId:t,knobIds:a}=m(n,this.config),s=t?n.states[t]:void 0,i=e?n.states[e]:void 0;if(!s&&!i){this.showNotice("No Patternflow entities found. Set device_id, or the entities, in the card configuration.");return}this.showNotice(null),this.dragging===null&&a.forEach((g,c)=>{let y=H(n.states[g]);y!==null&&this.pending[c]===null&&(this.local[c]=y)}),this.applyPattern(s),this.paintHead(s,i),this.paintOverlay(),this.paintPatterns(s);let l=i?.state==="off";this.stage?.classList.toggle("asleep",l),this.sandbox?.setRunning(!l&&this.config.preview!==!1)}applyPattern(n){let e=n?.attributes?.slug??null,t=n?.attributes?.knob_labels;if(Array.isArray(t)&&t.length===h&&(this.labels=t.map(String)),e===this.slug)return;this.slug=e;let a=e?E[e]??"":"";if(this.code=a,!a||this.config.preview===!1){this.sandbox?.disconnect(),this.sandbox?.element.remove(),this.sandbox=void 0,this.setStill(e?"No preview bundled for this pattern":"Presets have no preview \u2014 the controls still work");return}let s=S(a);this.ranges=s.ranges,this.labels=s.labels;let i=T(P(a))==="landscape";this.stage?.classList.toggle("landscape",i),this.sandbox||(this.sandbox=new u(W,l=>{l.ok||this.setStill(l.error??"This pattern did not load")}),this.sandbox.element.classList.add("frame"),this.stage?.prepend(this.sandbox.element),this.sandbox.connect()),this.setStill(null),this.sandbox.load(a,this.patternUnits(),this.ranges,[...v],x(this.ranges))}patternUnits(){return this.local.map((n,e)=>{let[t,a]=this.ranges[e]??[0,1];return t+n/100*(a-t)})}pushKnobsToSandbox(){this.sandbox?.setKnobs(this.patternUnits(),this.ranges)}paintHead(n,e){let t=this.root.querySelector(".title"),a=this.root.querySelector(".power"),s=this.root.querySelector(".badge"),i=n?.state??"\u2014";t.textContent=i,e&&(a.checked=e.state==="on");let l=!!n?.attributes?.absolute_ready;s.hidden=!n,s.textContent=l?"absolute":"relative"}paintOverlay(){this.zones.forEach((s,i)=>s.classList.toggle("active",i===this.active)),this.paintStrip();let n=this.root.querySelector(".readout-label"),e=this.root.querySelector(".readout-value"),t=this.root.querySelector(".fill"),a=this.labels[this.active]??`K${this.active+1}`;n.textContent=a===`K${this.active+1}`?a:`K${this.active+1} ${a}`,e.textContent=`${Math.round(this.local[this.active])}%`,t.style.width=`${Math.round(this.local[this.active])}%`}paintStrip(){let n=this.knobsUsable();this.knobButtons.forEach((e,t)=>{let a=Math.round(this.local[t]),s=this.labels[t]??`K${t+1}`,i=s===`K${t+1}`;e.classList.toggle("active",t===this.active),e.disabled=!n,e.title=i?s:`K${t+1} ${s}`;let l=e.querySelector(".knob-name"),g=e.querySelector(".knob-value"),c=e.querySelector(".knob-fill");l.textContent=i?s:`${t+1} ${s}`,g.textContent=n?`${a}%`:"\u2014",c.style.width=`${a}%`})}paintPatterns(n){let e=this.root.querySelector(".patterns");if(this.config.show_patterns===!1||!n){e.hidden=!0;return}e.hidden=!1;let t=n.attributes.options??[],a=n.state,s=`${t.join("\0")}${a}`;e.dataset.signature!==s&&(e.dataset.signature=s,e.innerHTML="",t.forEach(i=>{let l=document.createElement("button");l.type="button",l.className=i===a?"pattern current":"pattern",l.textContent=i,l.addEventListener("click",()=>{this.hassRef?.callService("select","select_option",{entity_id:n.entity_id,option:i})}),e.append(l)}))}setStill(n){let e=this.root.querySelector(".still");e.hidden=n===null,n!==null&&(e.textContent=n)}showNotice(n){let e=this.root.querySelector(".notice");e.hidden=n===null,n!==null&&(e.textContent=n)}};var f=class extends HTMLElement{constructor(){super();this.config={type:"custom:patternflow-card"};this.root=this.attachShadow({mode:"open"})}setConfig(n){this.config=n,this.render()}set hass(n){this.hassRef=n,this.render()}devices(){let n=this.hassRef;if(!n)return[];let e=new Set;for(let t of Object.values(n.entities??{}))t.platform==="patternflow"&&t.device_id&&e.add(t.device_id);return[...e].map(t=>({id:t,label:n.devices?.[t]?.name_by_user??n.devices?.[t]?.name??t}))}emit(n){this.config={...this.config,...n},this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this.config},bubbles:!0,composed:!0}))}render(){let n=this.devices(),e=this.config.device_id??n[0]?.id??"";this.root.innerHTML=`
      <style>
