...
- **Screw-terminal power** — strip a USB cable, clamp two wires into the 2-pin terminal on the back of the board. No soldering iron needed for the power input.
  > ⏸️ **Update (2026-07):** the board also carries a USB-C footprint (`USB1`), but **that input isn't running reliably yet and is on hold** while it's fixed and re-tested. Leave `USB1`/`R1`/`R2` unpopulated and power the board through `J4` for now. See [BUILD_GUIDE §2](https://github.com/engmung/Patternflow/blob/main/BUILD_GUIDE.md#2-power-input--use-the-screw-terminal) and [#114](https://github.com/engmung/Patternflow/issues/114).
