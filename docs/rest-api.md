...
**Spec version: 1.5 (unreleased additions)** · the released 1.4 contract applies to firmware **≥ 3.5.1**, with later endpoints dated in the version history below. Fields and routes marked 1.5 require a build containing the unreleased changes.

Clients should tolerate unknown status fields and probe capabilities before calling edition-specific routes.
...
| Advertised over mDNS | `_http._tcp` on port 80 and `_arduino._tcp` when `PF_OTA_ENABLED`. In the 1.5 implementation `core_names.h` owns startup and retry; in released 1.4 these are registered by the web updater and ArduinoOTA. HTTP carries no TXT records and OTA carries the Arduino discovery fields (board type, auth flags), with nothing Patternflow-specific. A discovering client must probe `GET /api/status` to confirm what it found. |
...
| Endpoint | Meaning |
|---|---|
| `GET /api/clock` | Every setting — `on`; `tz`, a POSIX TZ string (`KST-9`, `CET-1CEST,M3.5.0,M10.5.0/3` — note the inverted sign, and that this form carries the DST rule); `h12` (no AM/PM marker); `rot` (quarter turns of the panel; 1 is upright, the way the device's own menus read, and the default; 0 is the wide way); `face` (an index into `faces`, the names of the typefaces compiled in); `gap` (px between the rows upright, between the pairs wide); `sep` (what sits between: 0 nothing, 1 a bar cut from the pattern exactly as the digits are — across, the face's own colon instead of a bar); `sepw` (the bar's thickness); `in` (inside the digits: 0 the pattern, 1 the ink colour); `out` (outside them: 0 the pattern at `dim` percent, 1 the `bg` colour); `ink` and `bg` as `RRGGBB`; `fade` (crossfade the minute change) — plus `w`/`h` (the panel's native size), `glyphsRev` (the digit set compiled in), and what the panel currently believes: `synced`, `time` (`HH:MM:SS`), `today`, `zone` (the abbreviation in force, so summer time is visible). |
...
| `POST /api/wifi/reconnect` | Since 1.5. Replies `{ok:true,reconnecting:true}`, then requests a reconnect after 500 ms using the current credentials. Does not reboot or rewrite NVS. The link briefly drops; poll status after reconnecting rather than assuming the reply means it is already back. |
