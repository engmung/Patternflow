...
**Opening an HTML console page pauses the pattern, and polling can keep it paused.** A resident `.pfm` module and the console cannot both have the internal DRAM they need, so `GET /`, `/status`, `/patterns`, `/wifi` and `/mqtt` evict the running module, serve a short interstitial, and restore the pattern once the console has been **idle for 25 s**. The JSON endpoints under `/api/` do not evict anything, and neither does `/audio`. An automated client must therefore only ever touch `/api/*`; fetching `/` or `/status` on a schedule would leave the panel dark.

The second half of that rule is the one that catches people, and it is not visible from the endpoint list at all: **several `/api/` endpoints refresh the idle timer**, even though none of them causes a pause. `GET /api/mqtt`, `GET /api/patterns`, `GET /api/patterns/file` and `POST /api/sleep` all call the firmware's `noteConsoleApiCall()`, which resets the 25-second clock whenever a pause is already in effect. Poll any of them faster than every 25 s and the pause never expires — the pattern stays evicted long after the browser tab that caused it is closed, and the panel reads as hung with every other field looking healthy.

**`GET /api/status` is the only endpoint that never touches the timer.** So the rule for a polling client is: watch `status.consolePaused`, and while it is `true`, ask for nothing else. Nothing is lost by waiting — the pattern is not running, so its knobs are not moving either. `POST /api/patterns/select` is the exception worth knowing: it clears the pause outright, because an explicit pick is meant to win.

This is the honest price of the console, not a bug to route around — it was removed for one day, when serving whole pages at module-resident heap passed in sequential tests, and put back the same day because a real browser's parallel requests on this one-connection server locked the device up within seconds. Sending a person to `http://patternflow.local/` is fine and expected; polling it is not.
...
| `consolePaused` | Somebody has a console page open and the pattern is evicted. Not an error. |
