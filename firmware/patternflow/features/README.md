...
| `drawOverlay(frame)` | after the pattern draws, before present | scheduler clock, weather clock, the clock feature |
...
| `clock/` | a wall clock over the running pattern, `/clock`, `/api/clock` | The smallest feature here, and the one to copy first: settings in its own NVS namespace, a page, an overlay. Owns the time zone as a POSIX string with its DST rule; the core keeps the time (`src/core_clock.h`), this shows it. |
