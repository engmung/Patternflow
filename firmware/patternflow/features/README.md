...
| `clock/` | a wall clock composed over the running pattern, `/clock`, `/api/clock` | Settings in its own NVS namespace, a page with a live preview, and the first user of `composeFrame`: anti-aliased digits (a typeface rasterised offline by `toolchain/build_clock_glyphs.py`), seven-segment digits, or the pattern showing only inside huge digits. Owns the time zone as a POSIX string with its DST rule; the core keeps the time (`src/core_clock.h`), this shows it. |
