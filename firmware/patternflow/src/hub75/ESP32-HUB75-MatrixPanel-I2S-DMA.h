...
public:
  ...
  /* PATTERNFLOW ADDITION (not upstream) — blit a whole RGB888 frame.
   *
   * drawPixelRGB888() is built to change one pixel: for every call it bounds-
   * checks, works out which half of the panel the row is in, and then walks the
   * colour-depth planes recomputing the row pointer and doing a read-modify-write
   * per plane. Painting a full frame that way costs one call per pixel and two
   * RMWs per plane per word (the top and bottom halves arrive as separate calls
   * even though they share the same uint16_t).
   *
   * A full-frame repaint knows more than that. Both halves of a word are known at
   * once, the row pointers are the same for every pixel in a row, and nothing
   * needs bounds-checking. Measured on a 128x64 P2.5 at 8-bit depth this took the
   * frame's push cost from 12.3 ms to a fraction of it — see PFCanvas::present().
   *
   * `rgb` is W*H*3 bytes, row-major, 8 bits per channel, sized to the panel.
   * Colour output is identical to calling drawPixelRGB888() for every pixel: the
   * same lumConvTab/CIE1931 curve and the same bit masks are used.
   */
  void blitRGB888(const uint8_t *rgb,
