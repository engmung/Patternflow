  ...
  <div class="screen">
    ...
    <div class="stats">
      ...
      <!-- Which edition this panel is running. It was a line of small print in
           ...
           raises. -->
      <a class="stat" id="s-ed-block" href="https://patternflow.work/variants">
...
function tick(){
  ...
  fetch('/api/status',{cache:'no-store'}).then(function(r){return r.json()}).then(function(s){
    ...
    if(ed){
      ...
      $('s-ed-block').href='https://patternflow.work/variants#'+encodeURIComponent(ed);
    }else{
      $('s-ed').textContent='Patternflow';
      $('s-ed-sub').textContent='The standard firmware. Other editions add '+
        'sound or MQTT — tap to see them. Installing one is a click and keeps '+
        'your patterns and settings.';
      $('s-ed-block').href='https://patternflow.work/variants';
    }
