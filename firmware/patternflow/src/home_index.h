...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  #variant{margin-right:auto;margin-left:16px}
  #variant a{color:var(--muted);text-decoration:none;border-bottom:1px solid var(--rule)}
  #variant a:hover{color:var(--led);border-color:var(--led)}
...
// Core, or somebody's variant? Only the device can answer, so it does —
// and points at the shelf either way, because the useful question on a
// core panel is "what else could this run" and on a variant it is
// "what am I running and how do I get back".
function sayVariant(s){
  var el=$('variant');if(!el||!s)return;
  var v=s.variant||'core';
  var href='https://patternflow.work/variants'+(v==='core'?'':'#'+v);
  el.innerHTML='';
...
if(window.pfStatus){gate(window.pfStatus);sayVariant(window.pfStatus)}
