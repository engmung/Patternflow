...
async function measure(label: string, layers: Layer[]) {
  ...
  const stack = await buildStackAnnotation({ matrix, layers, activeLayerId: layers[0].id, knobs: [0, 0, 0, 0], ranges: [[0, 1], [0, 1], [0, 1], [0, 1]], knobLabels: ["a", "b", "c", "d"], forkOf: null, gen: { count: 5, thinking: "LOW", refs: 6, colorMode: "vfield" }, director: emptyShow() }).catch(() => null);
