...
export const ENTRIES: AtlasEntry[] = [
  ...
  { id: "vlasov", nm: "이중류 위상공간 소용돌이", nmEn: "Two-stream phase-space vortices", en: "BEAM-PLASMA ROLL-UP", f: "ink", st: "unexplored", x: 38, y: 39, added: "2026-09-05",
    tex: "서로를 향해 달리는 두 입자 빔이 자기 전기장에 걸려 위상공간 (x, v)에서 말려 올라간다 — 띠가 물결치고, 물결이 눈(소용돌이)이 되고, 눈들이 짝지어 합쳐진다. 화면 자체가 위상공간이다.",
    texEn: "Two particle beams running through each other catch on their own electric field and roll up in phase space (x, v) — bands ripple, ripples become eyes, eyes merge in pairs. The screen is phase space itself.",
    knob: "빔 속도/열 퍼짐 비 v0/vt = 0.5..6 — 따뜻한 빔은 Landau 감쇠로 잠잠 ↔ 문턱 ≈ 1.3 ↔ 차가운 빔이 소용돌이 사슬로 말린다 (이중류 불안정 문턱).",
    knobEn: "beam-to-thermal speed ratio v0/vt = 0.5..6 — warm beams stay Landau-damped and flat ↔ threshold ≈ 1.3 ↔ cold beams roll into a chain of phase-space vortices (the two-stream instability threshold)",
    vert: "세로축 = 속도. 위아래 두 띠가 빔, 그 사이 키 큰 눈들이 소용돌이 — 1:2 프레임이 위상공간의 자연스러운 종횡비.",
    vertEn: "The vertical axis IS velocity: the two beams are bands top and bottom, the tall eyes between them are the vortices — the 1:2 frame is phase space's own aspect.",
    risk: "병합 캐스케이드가 구멍 하나로 끝나면 정지 — 셀 수 있는 눈 하나. 파장을 여러 개 담고 빔을 계속 재주입할 것.",
    riskEn: "The merging cascade ends in a single hole and stalls — one countable eye. Fit many unstable wavelengths and keep re-injecting beam.",
    topic: "The two-stream instability as phase-space ink — a 1D electrostatic particle-in-cell plasma whose particles are deposited into an (x, v) buffer: x across the frame, v along the tall axis; two counter-streaming beams roll up into a chain of phase-space vortices that merge pairwise",
    hints: ["PIC in 1D: 4000-8000 macro-particles, charge deposited to 64 cells (cloud-in-cell), the field from one cumulative sum around the periodic ring (subtract the mean), leapfrog push with dt ≈ 0.1 in units where the plasma frequency is 1 — a few thousand flops per step, no transcendentals",
            "cold symmetric beams at ±v0 grow fastest near k·v0 ≈ 0.6, i.e. wavelength ≈ 10·v0 grid units: choose v0 so 5-8 wavelengths fit in 64 cells, and map v to y over about ±4·v0 so the vortices stand tall",
            "render by depositing every particle into a fading (x, v) buffer each step — the screen is the phase-space density, never the real-space one; the filamentary phase-mixing arms are the lace",
            "the cascade ends in one hole: each step reset a small random fraction of particles back onto the cold beams (a driven beam-plasma) so roll-up recurs forever"] },
  ...
  { id: "branched", nm: "분기 흐름 — 약한 무질서 속 광선 다발", nmEn: "Branched flow", en: "BRANCHED FLOW", f: "optics", st: "unexplored", x: 30, y: 58, added: "2026-09-05",
    tex: "평행하게 쏟아지는 광선들이 약하고 매끈한 무질서 퍼텐셜을 지나며 스스로 가지친다 — 렌즈도 초점도 없는데 밀도가 강줄기처럼 갈라지고 첨점이 태어난다. 퍼텐셜이 흐르면 가지가 다시 짜인다.",
    texEn: "Parallel rays pouring through a weak, smooth random potential branch on their own — no lens, no focus, yet the density splits into rivers and cusps are born. As the potential drifts, the branches re-knit.",
    knob: "퍼텐셜 세기 ε = 0..0.5 (광선 에너지 대비) — 곧은 평행류(분기 길이가 프레임보다 김) ↔ 첫 첨점이 프레임 안으로 들어옴(분기 길이 ∝ ε^(-2/3)) ↔ 다중 산란 포화.",
    knobEn: "potential strength epsilon = 0..0.5 (relative to the ray energy) — straight parallel flow, branching length longer than the frame ↔ the first cusps enter the frame (branching length scales as epsilon^(-2/3)) ↔ multiple-scattering saturation",
    vert: "위에서 아래로 내리는 광선의 전파축이 곧 긴 축 — 가지는 아래로 갈수록 자란다.",
    vertEn: "The propagation axis, top to bottom, is the tall axis — the branches grow richer with depth.",
    risk: "점광원 부채꼴은 '빔 하나'로 읽힌다 — 평행 발사에 느린 횡방향 스윕. 커스틱은 광선 밀도 발산 그 자체라 특별한 렌더가 필요 없다.",
    riskEn: "A point-source fan reads as one beam — launch parallel with a slow lateral sweep. The caustics are the ray-density divergence itself; no special render is needed.",
    topic: "Branched flow — thousands of rays launched parallel from the top edge, deflected by a weak smooth random potential as they fall, depositing into a fading ink buffer; density focuses into branching caustic rivers whose first cusps appear at a depth set by the potential strength",
    hints: ["paraxial ray step: each ray advances one row per step, its lateral velocity nudged by -dV/dx read from a coarse potential grid (about 16x32 nodes, bilinear), position updated — roughly ten flops per ray per step",
            "build V as a sum of a few slow low-frequency noises evaluated only on the coarse nodes (a few hundred trig per frame), drifting on incommensurate periods so the branch pattern re-knits; the potential's correlation length (4-24 px) sets branch spacing",
            "1000-2000 rays, 2-4 rows each per frame; recycle a ray that leaves the bottom back to the top at a seeded random x; fade buffer + peak EMA + smoothstep as usual"] },
  ...
  { id: "plastic", nm: "소성 흐름 수로 — 핀 무질서 속 디피닝", nmEn: "Plastic-flow channels", en: "PLASTIC DEPINNING", f: "advect", st: "unexplored", x: 64, y: 64, added: "2026-09-05",
    tex: "서로 밀치는 입자 격자가 얼어붙은 핀 무질서를 뚫고 아래로 끌려간다 — 어둠(고정) 사이로 빛의 강(움직임)이 열리고, 굽고, 막히고, 다른 곳에서 다시 터진다. 움직이는 것만 잉크를 남긴다.",
    texEn: "A lattice of mutually repelling particles dragged downward through frozen pinning disorder — rivers of light (motion) open between the dark (pinned), bend, choke, and burst open elsewhere. Only what moves leaves ink.",
    knob: "구동력 F = 0..2 (최대 핀 힘 단위) — 전면 고정(어둠) ↔ 디피닝 문턱 ↔ 소성 흐름(고정 섬 사이의 수로) ↔ 탄성 흐름(격자 전체가 함께 미끄러짐, 균일 줄무늬). 실재하는 두 동역학 전이.",
    knobEn: "driving force F = 0..2 (in units of the maximum pin force) — everything pinned (dark) ↔ depinning threshold ↔ plastic flow, channels winding between pinned islands ↔ elastic flow, the whole lattice sliding together as uniform streaks; two genuine dynamical transitions",
    vert: "구동이 긴 축을 따라 내려간다 — 수로는 세로 강.",
    vertEn: "The drive runs down the tall axis — the channels are vertical rivers.",
    risk: "creep(1D 전선 폭포)의 2D 형제. 점이 보이면 죽음 — 입자를 그리지 말고 속도로 잉크를 찍을 것.",
    riskEn: "The 2D sibling of creep (the 1D front waterfall). Dots showing is death — never draw the particles; deposit ink by speed.",
    topic: "Plastic flow of a driven particle lattice through quenched pinning — a few hundred short-range-repelling particles pushed down the frame through a frozen random pinning landscape, depositing ink only where they move; below the depinning threshold nothing moves, above it flow organizes into winding channels, and at high drive the lattice slides as a whole",
    hints: ["400-800 particles in a periodic 64x128 box; overdamped: velocity = drive + pair repulsion + pinning gradient; two substeps per frame",
            "pair repulsion through a cell list (8x16 cells, rebuilt by counting sort each step) and a force written in r-squared only — F = (dx, dy) * A * (1 - r2/rc2)^2 for r2 < rc2, rc ≈ 6 px — so no sqrt or division per pair",
            "pinning as a smooth random potential on a coarse grid (about 8x16 nodes), force by bilinear gradient — a seeded grid frozen for the whole run",
            "deposit ink proportional to speed squared into a fading buffer so pinned particles vanish and channels glow; if individual dots read, raise the count or blur the deposit"] },
  ...
  { id: "snowline", nm: "방향성 응고 전선", nmEn: "Directional solidification front", en: "MULLINS-SEKERKA FRONT", f: "pde", st: "unexplored", x: 29, y: 78, added: "2026-09-05",
    tex: "고정된 온도 기울기 속으로 시료가 당겨 내려간다 — 전선은 화면의 한 높이에 머물고, 그 위의 어두운 용융체로 세포와 수지상 끝이 자라 들어가며 갈라지고 서로를 밀어내 죽인다. 전선이 남긴 결정은 기록이 되어 아래로 흐른다.",
    texEn: "A sample pulled downward through a fixed temperature gradient — the front hovers at one height of the frame while cells and dendrite tips grow into the dark melt above it, splitting and crowding one another out. The crystal the front leaves behind flows down as a record.",
    knob: "당김 속도 V (D/l_T 단위) — 평면 전선 ↔ Mullins-Sekerka 임계 ↔ 세포 ↔ 곁가지 치는 수지상 (실재하는 형태 불안정 문턱). 이방성 ε4는 수지상 ↔ 끝갈림 해초를 따로 가른다.",
    knobEn: "pulling speed V (in units of D/l_T) — planar front ↔ the Mullins-Sekerka threshold ↔ cells ↔ side-branching dendrites (a genuine morphological instability); the anisotropy epsilon4 separately splits dendrites from tip-splitting seaweed",
    vert: "긴 축 = 당김 축. 전선은 위쪽 1/3에, 그 아래는 응고된 역사.",
    vertEn: "The tall axis is the pulling axis: the front sits in the upper third, everything below is solidified history.",
    risk: "눈송이 그림이 아니다 — 가운데 별 하나가 아니라 전선과 그 기록. 2차 위상장이지, 강성 때문에 기각된 6차 phase-field-crystal이 아니다.",
    riskEn: "Not a snowflake picture — a front and its record, never one centred star. This is the second-order phase field, not the sixth-order phase-field-crystal equation that was ruled out for stiffness.",
    topic: "Directional solidification — a phase field for solid/liquid plus a diffusing solute field, pulled down the frame through an imposed linear temperature gradient; solute rejected at the interface (constitutional undercooling) destabilizes the planar front into cells and dendrites while the gradient stabilizes it; the front holds one height and the frozen crystal scrolls down as a record",
    hints: ["two Float32 fields (phi and the solute U) evolved only in a band of about 40 rows around the front; below the band the solid is frozen and simply scrolls (integer row copies at the pulling rate), above it fresh melt enters at U = 0 — the band is what keeps this inside the budget",
            "canonical: interface width W = 1 (spans 3-4 cells), tau = 1, solute diffusivity D ≈ 2, coupling lambda ≈ 3, cubic anisotropy epsilon4 = 0.05; explicit stepping at dt ≈ 0.1..0.2 times dx^2/D; the thermal length above the front must fit in 30-50 rows",
            "anisotropy without atan2: cos(4 theta) = (gx^4 - 6 gx^2 gy^2 + gy^4) / (gx^2 + gy^2)^2 from the phi gradient — pure arithmetic",
            "render phi as the body with the gradient magnitude added as a bright interface seam, and a faint solute halo in the melt; the frozen record below the band needs only the row copy"] },

  { id: "vicinal", nm: "비시널 표면 — 계단 흐름과 뭉침", nmEn: "Vicinal step flow", en: "STEP FLOW", f: "pde", st: "unexplored", x: 21, y: 80, added: "2026-09-05",
    tex: "결정 표면의 계단들이 원자 비를 맞으며 아래로 흐른다 — 테라스는 평평한 판, 계단은 빛나는 등고선. 계단이 물결치고(meander), 뭉치고(bunch), 테라스 위에 섬이 핵생성해 계단과 합쳐진다. 거칠어짐 온도 위에서는 표면 전체가 끓는다.",
    texEn: "Steps on a crystal surface flowing downhill under a rain of atoms — terraces as flat plates, steps as glowing contour lines. Steps meander, bunch, islands nucleate on terraces and merge into them; above the roughening temperature the whole surface boils.",
    knob: "온도 kT/J = 0.3..1.8 — 각진 계단이 잠긴 평면(동결) ↔ 계단 흐름과 물결침 ↔ 거칠어짐 전이(Kosterlitz-Thouless 계열) 너머의 열적 끓음. 전이점은 이 범위 안에 있다 — 스윕해서 기본값을 그 바로 아래에 둘 것.",
    knobEn: "temperature kT/J = 0.3..1.8 — faceted steps locked flat ↔ step flow and meandering ↔ thermal boil beyond the roughening transition (Kosterlitz-Thouless class); the transition lies inside this range — sweep it and set the default just below where the steps dissolve",
    vert: "계단이 프레임 폭을 가로지르고 긴 축을 따라 내려간다 — 등고선의 폭포.",
    vertEn: "Steps run across the width and flow down the tall axis — a waterfall of contour lines.",
    risk: "구동(증착) 없이는 평형 요동뿐 — 원자 비와 Ehrlich-Schwoebel 장벽이 계단 물결침·뭉침을 낳는다. 높이가 자라면 정수 빼기로 되돌릴 것.",
    riskEn: "Without deposition it is only equilibrium flicker — the atom rain and an Ehrlich-Schwoebel barrier are what breed meandering and bunching. Subtract an integer from all heights as the surface grows.",
    topic: "Solid-on-solid growth on a vicinal crystal surface — an integer height lattice tilted into a staircase along the tall axis, atoms deposited at random and hopping by Arrhenius rates from a small table, with an Ehrlich-Schwoebel barrier at step edges; steps flow downhill, meander and bunch, islands nucleate on terraces, and above the roughening temperature the surface boils",
    hints: ["Int16Array heights with a helical periodic boundary along y (wrapping adds the staircase offset) so the vicinal tilt survives; each event: pick a site, propose moving its top atom to a neighbour, accept with a table probability indexed by the change in bond count (about 17 entries) — integer math plus one lookup, thousands of events per frame",
            "deposition flux F (atoms per site per frame) and the Ehrlich-Schwoebel factor (an extra table for hops that descend a step) are the aliveness axes; step meandering (Bales-Zangwill) and bunching appear as they grow",
            "render step density — a site bright where its height differs from a neighbour's, through an EMA — over a dim terrace shade (height mod 4)/4; never the raw heights",
            "warm up a few hundred sweeps in setup so the staircase is already alive"] },

  { id: "adhesion", nm: "2D Burgers 충격파 그물", nmEn: "2D Burgers shock web", en: "ADHESION WEB", f: "pde", st: "unexplored", x: 84, y: 83, added: "2026-09-05",
    tex: "압력 없는 흐름이 스스로 가팔라져 벽이 된다 — 물질이 벽으로, 벽이 필라멘트로, 필라멘트가 매듭으로 모이는 세포 그물. 세포는 합쳐지고 매듭은 미끄러지며, 느린 저어줌이 새 세포를 낳는다. Creasefall(1D)의 2차원 본토.",
    texEn: "Pressureless flow steepening into walls — matter draining into walls, walls into filaments, filaments into knots: a cellular web whose cells merge and whose knots slide, fresh cells born of slow stirring. The 2D homeland of Creasefall.",
    knob: "점성 ν = 0.03..0.6 (격자 단위) — 유리처럼 매끈한 이랑 ↔ 유한시간 기울기 파국이 면도날 벽·매듭 그물을 만든다. 저어줌 파장은 세포 크기.",
    knobEn: "viscosity nu = 0.03..0.6 (grid units) — glassy smooth ridges ↔ the finite-time gradient catastrophe sharpens into a web of razor walls and knots; the stirring wavelength sets the cell size",
    topic: "The 2D Burgers equation with gentle random stirring — u_t + (u·grad) u = nu lap(u) + f — as a shock web: velocity drains into a cellular network of compression walls that merge and coarsen while forcing keeps birthing cells; render the negative divergence so walls and knots glow (the adhesion model)",
    hints: ["two Float32 velocity fields; first-order upwind for the advection term and a 5-point Laplacian — about 30 flops per cell per step, zero transcendentals; dt below dx/max|u|, 4-6 substeps per frame",
            "stirring f as a few slow low-wavenumber sines per component evaluated on a coarse grid and interpolated, drifting on incommensurate periods; amplitude around 0.5 grid/time",
            "render max(0, -div u) through a peak EMA and smoothstep — walls bright, knots brightest, the smooth cell interiors dark; clamp |u| as the divergence guard"] },

  { id: "superflow", nm: "초유체 소용돌이 방출", nmEn: "Superfluid vortex shedding", en: "QUANTIZED VORTEX STREET", f: "pde", st: "unexplored", x: 63, y: 93, added: "2026-09-05",
    tex: "밝고 평평한 응축체를 장애물이 가르며 지나간다 — 임계 속도 아래선 아무 흔적 없이(초유동), 위에선 양자화된 소용돌이 쌍이 어두운 구멍으로 떨어져 나와 거리를 이루고, 음파 잔물결이 퍼지고, 구멍들이 얽혀 헝클어진다.",
    texEn: "An obstacle cutting through a bright flat condensate — below the critical speed it leaves nothing (superflow); above it, quantized vortex pairs peel off as dark pits, form a street, ring with sound, and tangle.",
    knob: "장애물 속도 v/c = 0..1 (c = 음속) — 무흔적 초유동 ↔ 방출 임계 v_c ≈ 0.4 ↔ 소용돌이 거리 ↔ 혼돈 얽힘 (Landau 판정 계열의 실재 전이).",
    knobEn: "obstacle speed v/c = 0..1 (c = sound speed) — trace-free superflow ↔ shedding onset near v_c ≈ 0.4 ↔ a vortex street ↔ a chaotic tangle (a genuine transition of the Landau-criterion kind)",
    vert: "장애물이 긴 축을 따라 오르고, 소용돌이 거리가 그 뒤로 길게 남는다.",
    vertEn: "The obstacle climbs the tall axis and the vortex street trails long behind it.",
    risk: "Schrödinger장이지 Navier-Stokes 솔버가 아니다 — 리샘플 없음. 비용은 lle급: 32x64로 풀고 보간할 것.",
    riskEn: "A Schrödinger field, not a Navier-Stokes solver — nothing is resampled. Cost sits at lle's level: solve at 32x64 and interpolate up.",
    topic: "Vortex shedding in a 2D superfluid — the Gross-Pitaevskii equation i psi_t = -lap(psi)/2 + (|psi|^2 - 1) psi + V psi with a repulsive obstacle potential moving up the frame; above a critical speed the obstacle nucleates quantized vortex pairs that trail behind as a street and tangle; render |psi|^2 so vortex cores read as dark pits in a bright condensate",
    hints: ["real and imaginary Float32 fields, 5-point Laplacians, explicit staggered (Visscher) stepping with dt ≈ 0.1 at grid spacing 0.5 healing lengths; the coarse-grid clause applies: simulate at 32x64 and interpolate the render",
            "obstacle: a Gaussian bump of height 3-5 and radius 3-4 healing lengths, its centre moving up at speed v and wrapping; use the damped form (i - gamma) psi_t = H psi with gamma ≈ 0.02 so sound decays and vortex pairs can annihilate",
            "render |psi|^2 through a peak EMA — the flat bright background is part of the picture; the vortices are its holes. Do not normalize the background away",
            "guard with isFinite plus a cap on |psi|^2 and reseed to psi = 1"] },
  ...
  { id: "braid", nm: "땋인 수송 — 스스로 깎는 지형", nmEn: "Braided transport", en: "BRAIDED TRANSPORT", f: "critical", st: "unexplored", x: 66, y: 57, added: "2026-09-05",
    tex: "위에서 부은 흐름이 비탈을 내려가며 제 바닥을 깎고 쌓는다 — 흐름이 갈라지고 다시 만나 땋임이 되고, 사주가 자라 물길을 막고, 물길이 옆으로 터진다(avulsion). 끝나지 않는 재배열.",
    texEn: "Flow poured in at the top runs down a slope, cutting and depositing its own bed — it splits and rejoins into braids, bars grow and choke a channel, the channel bursts sideways (avulsion). Rearrangement without end.",
    knob: "수송 비선형 지수 m = 1..3 (Q_s ∝ (QS)^m) — m ≤ 1 은 판 흐름(하나의 밋밋한 시트) ↔ m > 1 에서 땋임이 태어난다 (Murray-Paola의 땋임 전이). 횡방향 수송 계수는 사주 이동 속도.",
    knobEn: "transport nonlinearity m = 1..3 in Q_s ∝ (Q S)^m — at m ≤ 1 a single flat sheet ↔ above 1 braiding is born (the Murray-Paola braiding transition); the lateral transport coefficient sets how fast bars migrate",
    vert: "물이 긴 축을 따라 내려간다 — 입구는 위, 출구는 아래. 라우팅은 위에서 아래로 한 번 훑는 단일 패스.",
    vertEn: "Water runs down the tall axis — inlet at the top, outlet at the bottom; routing is one top-to-bottom sweep.",
    risk: "강 지도로 읽히면 죽음 — 둑도 물가도 그리지 말고 유량장만 빛으로. m 의 거듭제곱은 셀당 Math.pow 금지 (테이블).",
    riskEn: "If it reads as a river map it is dead — draw no banks, only the discharge field as light. The power in the transport law must not be a per-cell Math.pow (table it).",
    topic: "Braided transport on a self-eroding slope — a cellular model: water entering the top row is routed to each cell's three downstream neighbours in proportion to slope, sediment moves as a nonlinear function of discharge times slope plus a lateral term, and the bed erodes or aggrades from the flux divergence; render the discharge field so braiding channels, bars and avulsions are the picture",
    hints: ["Murray-Paola cellular model: one top-to-bottom sweep per step — route water to the three lower neighbours (down-left, down, down-right) in proportion to their positive slopes, spread equally when none is lower; sediment flux Q_s = K (Q S)^m out of each cell plus a lateral flux proportional to the cross-slope; bed change = sediment in minus out; the bottom row drains everything",
            "compute x^m without Math.pow: x·x^(m-1) with x^(m-1) from a 256-entry log/exp table pair, or blend integer powers; all other math is add/multiply",
            "initial bed: a uniform slope down the frame plus seeded noise; supply water and sediment at the top on a slow breathing schedule (harmless axis); keep the mean slope by draining the outlet",
            "render log(1 + Q) through a peak EMA and smoothstep — channels bright, bars dark; keep the bed elevation invisible"] },
