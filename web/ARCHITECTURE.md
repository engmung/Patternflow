...
- **`src/lib/lab/capture/`** — Pattern Lab's output stage (the Capture panel): PNG stills and MP4/WebM clips at print and screen sizes, rendered by a second `LabEngine` inside a Web Worker so a 4K frame never touches the lab's main thread. Everything is client-side — WebCodecs encodes, `mediabunny` muxes (dynamically imported), the file is a blob download; the server only serves the bundle. `probe.ts` decides what *Auto* means for a given pattern: it re-renders the stack at an aspect-true multiple and compares texture density and layout against the panel-size render, because most patterns bake the 128×64 frame into their math and would come back tiled or cropped. Those get a pixel upscale (and a copy-paste prompt that asks an AI to rewrite them in frame-relative units); the ones that scale get a true re-render. The module is an add-on: `engine.ts` gained one read-only `coverage()` query for transparent exports, and `PatternLabClient.tsx` registers the panel — nothing else in the lab knows it exists.
