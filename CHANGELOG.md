...
The release where the firmware stops being one thing. Features that not everybody wants can now leave the core and live in somebody else's repository, and this is the first release you can leave *for* one — [patternflow-audio](https://github.com/engmung/patternflow-audio) exists, has a binary, and puts OSC, the browser audio path and on-board microphone support in one firmware for people who point sound at a panel.

Nothing is taken away here. Core still ships everything it shipped in 3.6.3.

### Added
- **A variant is directories and two files, and it never edits a core one.** `addons_local.h` says which addons a firmware has, in what order, and may drop or reorder the defaults; `overrides.h` reaches any `#ifndef`-guarded setting — transmit power, panel clock, the firmware's own name — because it is included before any default applies. Both are gitignored here and belong to whoever built the variant, which makes taking a core update a file copy rather than a merge, forever. Measured three ways: default 1,410,501 B, a sound-only build 1,132,117 B, `PF_ADDONS_NONE` 1,092,045 B.
